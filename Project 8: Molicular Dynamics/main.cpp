...
int main() {
    ...
    for (int i = 0; i < 20; i++) {
        for (int j = 0; j < 20; j++) {
            ...
            initialPositions.push_back({i * 1.1 + 5.0 + rand() / RAND_MAX * 0.7 , j * 1.1 + 5.0 + rand() / RAND_MAX * 0.7});  // Place particles in a grid with spacing of 1.5 sigma
        }
    }


    unsigned int timeSteps = 300000;
